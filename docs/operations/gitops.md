...
当前仓库已经把 Flux/SOPS/GitOps 放进默认后端工程路径；`web-bff`、`outbox-relay-worker`、`projector-worker` 都已接入 shared counter DB secret，但它们当前仍不应被当成“完全闭环的默认生产交付模板”。`counter-service` 的独立 deployable、共享 worker 可靠性语义，以及后续 promotion/drift 闭环仍需继续补齐。
