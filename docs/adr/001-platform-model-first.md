...
- `platform/model/` is the declared index for platform metadata, deployables, topologies, workflows, resources, and environments
...
### Rationale
1. **Consistency**: A declared model reduces drift between code, infrastructure, and docs
2. **Automation**: Changes to the model can propagate to generated consumers when generators and drift checks are current
3. **Verification**: Model declarations can be validated against schemas before generating anything
