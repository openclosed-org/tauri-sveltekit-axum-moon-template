...
pub async fn init_tenant(
    ...
    let existing: Vec<UserTenantRecord> = admin_db
        .query(
            ...
            BTreeMap::from([(
                "sub".into(),
                surrealdb::types::Value::String(body.user_sub.clone()),
            )]),
        )
        .await
    ...
    let created_tenants: Vec<TenantRecord> = admin_db
        .query(
            ...
            BTreeMap::from([(
                "name".into(),
                surrealdb::types::Value::String(tenant_name.clone()),
            )]),
        )
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let tenant_record_id =
        RecordId::parse_simple(&tenant_id).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let _: Vec<serde_json::Value> = admin_db
        .query(
            ...
            BTreeMap::from([
                ...
                (
                    "tid".into(),
                    surrealdb::types::Value::RecordId(tenant_record_id),
                ),
            ]),
        )
        .await
